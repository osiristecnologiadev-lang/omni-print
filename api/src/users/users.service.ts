...
import { defaultPermissionsFor, validatePermissionsForScope } from '../auth/permissions.util';
...
export class UsersService {
  ...
  async updatePermissions(tenantId: string, userId: string, permissions: string[]) {
    const user = await this.prisma.user.findFirst({ where: { id: userId, tenantId } });
    if (!user) {
      throw new NotFoundException('user not found');
    }
